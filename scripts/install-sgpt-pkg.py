...
def create_bind_script():
    ...
    with open(PROFILE_SCRIPT_LOCATION, "w") as f:
        f.write(r"""#!/bin/bash
# hardcode the path to sgpt (aliases are unreliable inside functions)
_sgpt_cmd="/usr/bin/sgpt.sh"

# Only run in interactive Bash sessions
if [[ $- == *i* ]] && [[ -n "$BASH_VERSION" ]]; then
  # optional: make sure Readline keymap is active
  set -o emacs 2>/dev/null

  
  _sgpt_bash() {
    # called only by bind -x; READLINE_LINE is set in that context
    if [[ -n "$READLINE_LINE" ]]; then
      # pass the current line to sgpt and replace it with the response
      READLINE_LINE="$("$_sgpt_cmd" sh "$READLINE_LINE" --stream)"
      READLINE_POINT=${#READLINE_LINE}
    fi
  }

  # Warning: overrides the usual Ctrl+L (clear-screen)
  bind -x '"\C-l": _sgpt_bash'
fi

# alias can remain for normal use in the terminal
if [[ -x "$_sgpt_cmd" ]]; then
